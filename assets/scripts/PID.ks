...
function PID_seek {
  ...
  local oldInput is PID_array[10]. // prev return value, just in case we have to do nothing and return it 
								   //again.
  ...
  if oldT < 0 {
    ...
    if dT > 0 { // Do nothing if no physics tick has passed from prev call to now.
     ...
     if (oldI > 0 or onlyPD > cMin) and (oldI < 0 or onlyPD < cMax) { // only do the I turm when within 
																	  //the control range
